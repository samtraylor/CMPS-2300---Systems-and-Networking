 ...
 * fork that isn't theirs (mutual exclusion), and will die if they don't eat
...
pthread_mutex_t locks[5] = {PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER};
pthread_mutex_t pickup = PTHREAD_MUTEX_INITIALIZER;
...
{
	//("Philosopher %d About to start meal, Fork Array is as such: { %d, %d, %d, %d, %d }\n",philosopher_number,forks[0],forks[1],forks[2],forks[3],forks[4]);
	printf("Philosopher %d has started her meal!\n",philosopher_number);


	if((philosopher_number % 2) == 0) //EVEN #'S LOOK LEFT FIRST
	{ 
  		...
  		if(forks[philosopher_number] < 1)
		{
	    	...
	    	return 1;
		  	...
		  	pthread_mutex_lock(&locks[philosopher_number]);
	    	forks[philosopher_number]--;
	    	
	    	//printf("Fork Array after philosopher %d took first fork is as such: { %d, %d, %d, %d, %d }\n",philosopher_number,forks[0],forks[1],forks[2],forks[3],forks[4]);
		...
		if(forks[(philosopher_number+1)%5] < 1)
		{
			...
			pthread_mutex_lock(&locks[(philosopher_number + 1)%5]);
	    	forks[(philosopher_number+1)%5]--;
	    	
	    	//printf("Fork Array after philosopher %d took second fork is as such: { %d, %d, %d, %d, %d }\n",philosopher_number,forks[0],forks[1],forks[2],forks[3],forks[4]);
...
void return_forks(int philosopher_number)
{
  ...
  forks[(philosopher_number + 1)%5]++;
  forks[philosopher_number]++;
  pthread_mutex_unlock(&locks[(philosopher_number + 1)%5]);
  pthread_mutex_unlock(&locks[philosopher_number]);
...
void *Philosopher(void *num)
{
  ...
  while(1)
  {
	...
	pthread_mutex_unlock(&pickup); //TRY
  }
}
